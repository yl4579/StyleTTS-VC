...
def main(config_path):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            gt.requires_grad_()
            out, _ = model.discriminator(gt.unsqueeze(1), labels)
            loss_real = adv_loss(out, 1)
            loss_reg = r1_reg(out, gt)

            out, _ = model.discriminator(mel_rec.detach().unsqueeze(1), labels)
            ...
            if epoch > TMA_epoch and epoch < VC_epoch: # start TMA training
                loss_s2s = 0
            ...
            # adversarial loss
            with torch.no_grad():
                _, f_real = model.discriminator(gt.unsqueeze(1), labels)
            out_rec, f_fake = model.discriminator(mel_rec.unsqueeze(1), labels) # reconstructed sample
            ...
            # norm and F0 loss
            if epoch >= VC_epoch: # start VC training (for norm and F0 matching loss)
                # norm loss
                fake_norm = log_norm(mel_fake.unsqueeze(1)).squeeze(1)
                loss_norm = ((torch.nn.ReLU()(torch.abs(fake_norm - real_norm) - 0.5))**2).mean()
                ...
                mel_cyc = model.decoder(en, F0_fake, fake_norm, s)
                
                loss_cyc = criterion(mel_cyc, gt)
            ...
            # freeze text aligner and pitch extractor for VC training for simplicity
            if epoch > TMA_epoch and epoch < VC_epoch: 
                optimizer.step('text_aligner')
                optimizer.step('pitch_extractor')
                        
            iters = iters + 1
            if (i+1)%log_interval == 0:
                logger.info ('Epoch [%d/%d], Step [%d/%d], Mel Loss: %.5f, Adv Loss: %.5f, Disc Loss: %.5f, Mono Loss: %.5f, S2S Loss: %.5f, Style Loss: %.5f, Feat Loss: %.5f, Cycle Loss: %.5f'
                        %(epoch+1, epochs, i+1, len(train_list)//batch_size, running_loss / log_interval, loss_adv.item(), d_loss.item(), loss_mono, loss_s2s, loss_sty, loss_feat, loss_cyc))
                ...
                writer.add_scalar('train/style_loss', loss_sty, iters)
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(val_dataloader):
                ...
                mel_fake = model.decoder(en, F0_real, real_norm, s_trg)
                
                fake_norm = log_norm(mel_fake.unsqueeze(1)).squeeze(1).detach()
                F0_fake, _, _ = model.pitch_extractor(mel_fake.unsqueeze(1))
                mel_cyc = model.decoder(en, F0_fake, fake_norm, s)
                loss_cyc = criterion(mel_cyc, gt)                
        ...
        logger.info('Validation cycle loss: %.3f' % (loss_test / iters_test))
        print('\n\n\n')

        writer.add_scalar('eval/cycle_loss', loss_test / iters_test, epoch + 1)
        attn_image = get_image(s2s_attn[0].cpu().numpy().squeeze())
        writer.add_figure('eval/attn', attn_image, epoch)
        mel_image = get_image(mel_cyc[0].cpu().numpy().squeeze())
